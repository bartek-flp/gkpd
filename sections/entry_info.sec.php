...
if ($entry_window_open < 2) $header1_8 .= sprintf("<a name=\"categories\"></a><h2>%s</h2>",$label_styles_accepted);
...
if (strpos($_SESSION['prefsStyleSet'],"BABDB") !== false) {
	...
	do {
		...
		if (!empty($row_styles['brewStyleEntry'])) {
			...
			$brewStyleInfo = $row_styles['brewStyleInfo'];//str_replace("<p>","",$row_styles['brewStyleInfo']);
			...
			$style_info_modals .= "\t\t\t\t<h4 class=\"modal-title\" id=\"assignment-modal-label-".ltrim($row_styles['brewStyleGroup'], "0").$row_styles['brewStyleNum']."\">Informacja o ".$row_styles['brewStyle']."</h4>\n";
			...
			$style_info_modals .= "\t\t\t\t<button type=\"button\" class=\"btn btn-danger\" data-dismiss=\"modal\">Zamknij</button>\n";
