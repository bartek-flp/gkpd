...
if ($section != "admin") {
	...
	if (!$logged_in) {	
		...
		if (!$logged_in) {	
			...
			if (($registration_open != 1) && ($judge_window_open == 1) && (!$judge_limit) && (!$steward_limit)) $page_info100 .= sprintf("%s otwarta od %s do %s.", $sidebar_text_000, $judge_open_sidebar, $judge_closed_sidebar);
			elseif (($registration_open != 1) && ($judge_window_open == 1) && ($judge_limit) && (!$steward_limit)) $page_info100 .= sprintf("<a href=\"%s\">%s</a> otwarta od %s do %s.", build_public_url("register","steward","default","default",$sef,$base_url), $sidebar_text_001, $judge_open_sidebar, $judge_closed_sidebar);
			elseif (($registration_open != 1) && (!$judge_limit) && ($judge_window_open == 1) && ($steward_limit)) $page_info100 .= sprintf("<a href=\"%s\">%s</a> otwarta od %s do %s.", build_public_url("register","judge","default","default",$sef,$base_url), $sidebar_text_002, $judge_open_sidebar, $judge_closed_sidebar);
			elseif (($registration_open == 2) && ($judge_window_open == 1) && ($judge_limit) && ($steward_limit)) $page_info100 .= sprintf("%s",$sidebar_text_003);
			else {
				if ($registration_open == 1) $page_info100 .= sprintf("%s %s do %s.", $sidebar_text_005, $reg_open_sidebar, $reg_closed_sidebar);
				else $page_info100 .= sprintf("%s %s do %s.", $sidebar_text_005, $reg_open_sidebar, $reg_closed_sidebar);
			}
			$page_info100 .= "</div>";
			$page_info100 .= "</div>";
		}
	...
	if ($show_entires) {
		...
		if ((!$comp_entry_limit) && (!$comp_paid_entry_limit)) $page_info200 .= sprintf("%s %s do %s.", $sidebar_text_009, $entry_open_sidebar, $entry_closed_sidebar);
		...
		if ($_SESSION['prefsShipping'] == 1) {
			...
			$header1_500 .= "<h4 class=\"panel-title\">Recepcja piw";
			if ($shipping_window_open == 1) $header1_500 .= " Dostępna";
			else $header1_500 .= " Niedostępna";
			$header1_500 .= "</h4>";
			$header1_500 .= "</div>";
