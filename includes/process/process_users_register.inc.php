		...
		if ($totalRows_userCheck > 0) {
			...
			if ($_SESSION['prefsEmailRegConfirm'] == 1) {
				...
				if ($show_entrant_fields) {
					...
					if (isset($_POST['brewerJudge'])) $message .= sprintf("<tr><td valign='top'><strong>%s:</strong></td><td valign='top'>%s<br>%s</td></tr>",$label_judge,$brewerJudge, "obowiązkowa obecność dla wszystkich sędziów w piątek 20.10 o godz. 13:00 w Warszawie, ul. Czerniakowska 126 a lok. 4");
				...
				$from_name = "Serwer konkursowy ".$_SESSION['contestName'];
				$from_email = $mm_from_email;//'mgr.trac@gmail.com';//"noreply@".$url;
				$mail = new PHPMailer(true);
				try {
				    //Server settings
				    $mail->SMTPDebug = 2;                                 // Enable verbose debug output
				    $mail->isSMTP();                                      // Set mailer to use SMTP
				    $mail->Host = $mm_server;  // Specify main and backup SMTP servers
				    $mail->SMTPAuth = true;                               // Enable SMTP authentication
				    $mail->Username = $mm_user;                 // SMTP username
				    $mail->Password = $mm_password;                           // SMTP password
				    $mail->SMTPSecure = $mm_secure;                       // Enable TLS encryption, `ssl` also accepted
				    $mail->Port = $mm_port;                                    // TCP port to connect to

				    //Recipients
				    $mail->setFrom($from_email, $from_name);
				    $mail->addAddress($to_email, $to_recipient);     // Add a recipient
