...
if ((isset($_SERVER['HTTP_REFERER'])) && ((isset($_SESSION['loginUsername'])) && ($_SESSION['userLevel'] <= 1))) {
		...
		if (($action == "add") || ($action == "edit")) {
			...
			foreach($_POST['score_id'] as $score_id)	{
				...
				$query_delete_assign = sprintf("SELECT id FROM $judging_scores_db_table WHERE eid='%s' AND scoreTable='%s'", $_POST['eid'.$score_id], $id);
				...
				if ((!empty($_POST['scoreEntry'.$score_id])) || (($_POST['scoreEntry'.$score_id]) === '0') || (!empty($_POST['scoreMiniBOS'.$score_id])) || (!empty($_POST['scorePlace'.$score_id]))) {
