...
function score_entry_data($value, $tid) {
	
	require(CONFIG.'config.php');
	mysqli_select_db($connection,$database);
	
	$query_scores = sprintf("SELECT id,eid,bid,scoreEntry,scorePlace,scoreMiniBOS FROM %s WHERE eid='%s' and scoreTable='%s'", $prefix."judging_scores", $value, $tid);
...
function score_should_be_in_finals($eid) {
	require(CONFIG.'config.php');
	mysqli_select_db($connection,$database);
	
	//$query_scores = sprintf("SELECT id,eid,bid,scoreEntry,scorePlace,scoreMiniBOS FROM %s WHERE eid='%s' and scoreTable='%s'", $prefix."judging_scores", $value, $tid);
	$query_scores = sprintf("SELECT 1 from dual where exists(select * from %s s left join %s t on s.scoreTable=t.id where s.scoreEntry = 0 and t.tableIsFinal = 0 and s.eid='%s')", $prefix."judging_scores", $prefix."judging_tables", $eid);
...
function table_score_data($eid,$score_table,$suffix) {
	...
	$query_tables = sprintf("SELECT id,tableName,tableIsFinal,tableNumber FROM %s WHERE id='%s'", $prefix."judging_tables".$suffix, $score_table);
	$tables = mysqli_query($connection,$query_tables) or die (mysqli_error($connection));
	$row_tables = mysqli_fetch_assoc($tables);
	$totalRows_tables = mysqli_num_rows($tables);
